...
def test_get_users_superuser_me(
    ...
    r = client.get(
        f"{settings.API_VERSION_PREFIX}/users/me", headers=superuser_token_headers
    )
    current_user = r.json()
    assert current_user
...
def test_get_users_normal_user_me(
    ...
    r = client.get(
        f"{settings.API_VERSION_PREFIX}/users/me", headers=normal_user_token_headers
    )
    current_user = r.json()
    assert current_user
    assert current_user["is_active"] is True
...
def test_create_user_new_email(
    ...
    with (
        patch("src.utils.send_email", return_value=None),
...
def test_get_existing_user_permissions_error(
    ...
    r = client.get(
        f"{settings.API_VERSION_PREFIX}/users/d36290cb-42e4-4e8a-9d35-2cb5fc990612",
...
def test_retrieve_users(
    ...
    r = client.get(
        f"{settings.API_VERSION_PREFIX}/users/", headers=superuser_token_headers
    )
    all_users = r.json()
...
#     with patch("auth.src.core.config.settings.USERS_OPEN_REGISTRATION", True):
#         password = random_lower_string()

#         data = {"email": settings.FIRST_SUPERUSER_EMAIL, "password": password}
#         r = client.post(
#             f"{settings.API_VERSION_PREFIX}/users/signup",
#             json=data,
#         )
...
def test_update_user(
    ...
    data = {"first_name": "Updated_full_name"}
    r = client.patch(
    ...
    assert updated_user["first_name"] == "Updated_full_name"

    user_query = select(User).where(User.email == username)
    user_db = db.exec(user_query).first()
    db.refresh(user_db)
    assert user_db
    assert user_db.first_name == "Updated_full_name"
