...
class H2ogptConverseWithDocs(H2ogptConverse):
    ...
    async def build_pipelines(
        ...
        if not req.pipelines:
            ...
            print("using pipelines", req.pipelines)
    ...
    async def instruction_send(
        ...
        result = ast.literal_eval(result)["response"]
    ...
    async def converse_with_docs(self, req: ConverseWithDocsRequest) -> Any:
        document_choice = []
        pipeline = asyncio.create_task(self.build_pipelines(req))
